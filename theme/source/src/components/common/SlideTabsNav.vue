...
import IconMore from '@/components/icons/IconUser.vue';
...
export default {
  ...
  setup() {
    ...
    const navItems = [
      ...
      { title: 'Mine', path: '/mine', name: 'Mine', icon: 'IconMore', i18nKey: 'mine' },
    ];
    ...
    const handleWindowLoad = () => {
      ...
      safeTimeout(() => updateSliderPosition(index, true), 100);
    };
    const handlePopstate = () => {
      ...
      safeTimeout(() => {
        const index = findIndexByRouteName(route.name);
        updateSliderPosition(index, false);
        safeTimeout(() => updateSliderPosition(index, true), 50);
      }, 0);
    };
