...
class ConfigValidator {
  ...
  List<ValidationError> getValidationErrors(Map<String, dynamic> json) {
    ...
    // 检查基本结构 - 至少要有一个主要字段（domains 为新 Flux 格式）
    if (!json.containsKey('domains') &&
        !json.containsKey('panels') &&
      ...
      errors.add(ValidationError(
        field: 'root',
        message: 'Configuration must contain at least one of: domains, panels, proxy, ws, update, onlineSupport, subscription',
    ...
    // 验证其他服务字段（update 为 Map 格式的 UpdateRichConfig，不走数组验证）
    for (final key in ['proxy', 'ws']) {
...
class ConfigurationParser {
  ...
  ///
  /// 支持两种格式：
  ///   新格式（Flux 风格）：{"panel_type": "xboard", "domains": ["https://..."]}
  ///   旧格式：{"panels": {"xboard": [{"url": "...", "description": "..."}]}}
  Map<String, dynamic>? extractConfigFromRemoteResult(Map<String, dynamic> remoteResult) {
    // 新格式（Flux 风格）：domains 数组
    if (remoteResult.containsKey('domains')) {
      final domains = remoteResult['domains'] as List<dynamic>? ?? [];
      final panelType = (remoteResult['panel_type'] as String?)?.isNotEmpty == true
          ? remoteResult['panel_type'] as String
          : 'xboard';
      ...
      final normalized = <String, dynamic>{
        ...
        'panels': {
          panelType: domains
              .map((url) => {'url': url.toString(), 'description': url.toString()})
              .toList(),
        },
      };
  ...
  Map<String, dynamic> _mergeConfigurations(List<Map<String, dynamic>> configs) {
    ...
    for (final config in configs) {
      ...
      // 合并其他列表字段（update 为 Map，单独处理）
      for (final key in ['proxy', 'ws', 'onlineSupport']) {
      ...
      // update 为 Map 格式，后来者覆盖
      if (config.containsKey('update')) {
        merged['update'] = config['update'];
      }
