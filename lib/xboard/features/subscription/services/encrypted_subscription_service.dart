...
class EncryptedSubscriptionService {
  ...
  static Future<SubscriptionResult> getEncryptedSubscription(
    ...
    try {
      ...
      if (enableRace && (subscriptionInfo.urls.length > 1)) {
        _logger
            .info('[订阅竞速] 检测到 ${subscriptionInfo.urls.length} 个订阅源，启动竞速选择...');
  ...
  static Future<DataResult> _fetchEncryptedData(String url) async {
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          _logger
              .warning('[数据获取] ⏰ 请求超时，${attempt * 2}秒后进行第${attempt + 1}次重试...');
          await Future.delayed(Duration(seconds: attempt * 2));
          continue;
        } else {
        ...
        if (attempt < maxRetries) {
          _logger.warning(
              '[数据获取] ⚠️ 请求异常: $e，${attempt * 2}秒后进行第${attempt + 1}次重试...');
          await Future.delayed(Duration(seconds: attempt * 2));
          continue;
        } else {
  ...
  static Future<SubscriptionResult> fallbackToNormalSubscription(
    ...
    try {
      ...
      if (enableRace && (subscriptionInfo.urls.length > 1)) {
        ...
        normalUrl =
            subscriptionInfo.buildSubscriptionUrl(token, forceEncrypt: false);
      }
  ...
  static Future<SubscriptionResult> getSubscriptionSmart(
    ...
    try {
      ...
      if (preferEncrypt) {
        ...
        // 回退到普通订阅
        return await fallbackToNormalSubscription(token,
            enableRace: enableRace);
      } else {
        // 直接使用普通订阅
        return await fallbackToNormalSubscription(token,
  ...
  static Future<SubscriptionResult> getRaceEncryptedSubscriptionFromLogin({
    ...
    try {
      ...
      // 使用并发竞速服务
      return await ConcurrentSubscriptionService
          .raceGetEncryptedSubscriptionFromLogin(
        preferEncrypt: preferEncrypt,
      );
    } catch (e) {
  ...
  static Future<SubscriptionResult> getRaceEncryptedSubscription(
    ...
    try {
      _logger.info(
          '[竞速增强] 获取加密订阅，token: ${token.substring(0, 8)}..., 竞速模式: $enableRace');
