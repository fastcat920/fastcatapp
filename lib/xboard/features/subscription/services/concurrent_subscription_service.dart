...
class ConcurrentSubscriptionService {
  ...
  static Future<SubscriptionResult> _raceMultipleRequests(
    ...
    for (int i = 0; i < urls.length; i++) {
      ...
      futures.add(_fetchSingleSubscriptionWithCancel(
          url, originalToken, cancelToken, i));
    }
    ...
    try {
      ...
      final results = await Future.wait(
        futures.map((future) =>
            future.catchError((e) => SubscriptionResult.failure('请求失败: $e'))),
      );
  ...
  static Future<SubscriptionResult> _fetchSingleSubscriptionWithCancel(
    ...
    try {
      _logger.debug(
          '[竞速订阅] 请求 #$index 开始: ${url.length > 50 ? '${url.substring(0, 50)}...' : url}');
