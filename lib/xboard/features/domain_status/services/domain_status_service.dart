...
class DomainStatusService {
  ...
  Future<Map<String, dynamic>> checkDomainStatus() async {
    ...
    try {
      ...
      // 若远程配置尚未加载（面板列表为空），先从 OSS 加载远程配置。
      // 首次安装/无缓存时 XBoardConfig.initialize() 使用 autoWarmUp:false，
      // 不会自动拉取远程配置，导致 allPanelUrls 为空 → 直接返回 null → "无可用域名"。
      // 这是 V2Board 首次使用必现的问题（XBoard 因有历史缓存而正常）。
      if (XBoardConfig.allPanelUrls.isEmpty) {
        _logger.info('面板列表为空，先加载远程配置...');
        try {
          await XBoardConfig.refresh();
          _logger.info('远程配置加载完成，面板数: ${XBoardConfig.allPanelUrls.length}');
        } catch (e) {
          _logger.error('加载远程配置失败，继续尝试竞速（可能使用过期缓存）', e);
        }
      }
