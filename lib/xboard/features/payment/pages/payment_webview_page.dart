...
/// - Linux → flutter_inappwebview (embedded InAppWebView)
///
...
class PaymentWebViewPage extends ConsumerStatefulWidget {
  ...
  /// No-op on Linux: InAppWebView is embedded in the Flutter tree and
  /// responds to theme changes through the normal widget rebuild cycle.
  @override
...
class _PaymentWebViewPageState extends ConsumerState<PaymentWebViewPage> {
  ...
  /// Linux uses flutter_inappwebview (InAppWebView widget) instead.
    ...
    if (_useSystemWebView) {
      ...
      webView = iaw.InAppWebView(
        initialUrlRequest: iaw.URLRequest(
          url: iaw.WebUri(widget.paymentUrl),
        ),
        ...
        onLoadStop: (controller, url) {
          if (mounted) _finishPageLoading();
        },
        shouldOverrideUrlLoading: (controller, navigationAction) async {
          final uri = navigationAction.request.url;
          if (uri == null) return iaw.NavigationActionPolicy.ALLOW;
          final scheme = uri.scheme.toLowerCase();
          if (_isStandardWebScheme(scheme)) return iaw.NavigationActionPolicy.ALLOW;
          await _launchExternalPaymentUri(uri);
