...
/// Linux 桌面端 Crisp 客服页。
///
/// 使用 CEF texture 在当前 Flutter 页面内承载 Crisp。
class LinuxCefCrispChatPage extends StatefulWidget {
  final String websiteId;
  final String? crispProxyUrl;
  final String? userScript;
  ...
  @override
  State<LinuxCefCrispChatPage> createState() => _LinuxCefCrispChatPageState();
}
...
class _LinuxCefCrispChatPageState extends State<LinuxCefCrispChatPage> {
  ...
  HttpServer? _bootstrapServer;
  Future<Uri>? _bootstrapServerFuture;
  Timer? _embedFallbackTimer;
  ...
  void didChangeDependencies() {
    ...
    if (_isDarkMode != nextIsDarkMode) {
      _isDarkMode = nextIsDarkMode;
      unawaited(_applyCefTheme());
    }
    if (_localeTag != nextLocaleTag) {
      _localeTag = nextLocaleTag;
      unawaited(_applyCefLocale());
    }
    if (!_didStartLoading) {
      _didStartLoading = true;
      _loadPreferredEmbed();
  ...
  void dispose() {
    _embedFallbackTimer?.cancel();
    unawaited(_bootstrapServer?.close(force: true));
  ...
  Future<cef.WebViewController> _ensureController({
    required String initialUrl,
  }) {
    if (_controllerInitialized && _controller != null) {
      return Future.value(_controller!);
    }
    return _controllerFuture ??= _createController(initialUrl: initialUrl);
  }

  Future<cef.WebViewController> _createController({
    required String initialUrl,
  }) async {
    ...
    controller.setWebviewListener(
      cef.WebviewEventsListener(
        ...
        onConsoleMessage: (level, message, source, line) {
          if (level >= 3) {
            debugPrint(
              '[Crisp][LinuxCEF] console[$level] $source:$line $message',
            );
          }
        },
      ),
  ...
  Future<void> _loadUrl(String url) async {
    final shouldLoadAfterInit = _controllerInitialized;
    final controller = await _ensureController(initialUrl: url);
    if (!mounted) return;
    if (shouldLoadAfterInit) {
  ...
  Future<void> _loadBootstrap() async {
    ...
    _embedFallbackTimer = Timer(
      ...
      () => unawaited(_handleBootstrapTimeout()),
    );
    try {
      final uri = await _ensureBootstrapServer();
      debugPrint('[Crisp][LinuxCEF] loadUrl bootstrap: $uri');
      await _loadUrl(uri.toString());
    } catch (_) {
      _fallbackFromBootstrapToEmbed();
    }
  }

  Future<Uri> _ensureBootstrapServer() {
    final existing = _bootstrapServerFuture;
    if (existing != null) return existing;
    return _bootstrapServerFuture = _startBootstrapServer();
  }

  Future<Uri> _startBootstrapServer() async {
    final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
    _bootstrapServer = server;
    unawaited(_serveBootstrapRequests(server));
    return Uri(
      scheme: 'http',
  ...
  Future<void> _serveBootstrapRequests(HttpServer server) async {
    try {
      await for (final request in server) {
        try {
          ...
          request.response.write(_buildBootstrapHtml());
          await request.response.close();
        } catch (_) {
  ...
  Future<void> _handleBootstrapTimeout() async {
    if (!mounted || !_usingBootstrap) return;
    if (await _isCrispReady()) return;
    _fallbackFromBootstrapToEmbed();
  }

  void _fallbackFromBootstrapToEmbed() {
    if (!mounted || _didFallbackToEmbed) return;
    _didFallbackToEmbed = true;
    final preferred = _preferredEmbedUri();
    final usingProxy = isCrispProxyConfigured(widget.crispProxyUrl);
    _loadEmbed(preferred, usingProxy: usingProxy);
  }
  ...
  void _loadEmbed(Uri uri, {required bool usingProxy}) {
    ...
    debugPrint('[Crisp][LinuxCEF] loadUrl embed: $uri');
    unawaited(_loadUrl(uri.toString()).catchError((_) {
      _handleRouteError();
    }));
  }
  ...
  Future<bool> _isCrispReady() async {
    final controller = _controller;
    if (controller == null || !_controllerInitialized) return false;
    try {
...
<body>
  ...
  Future<void> _injectDirectEmbedMonitor() async {
    final controller = _controller;
    if (controller == null || !_controllerInitialized) return;
    final background = _customerServiceBackgroundColorValue(_isDarkMode);
    final foreground = _customerServiceForegroundColorValue(_isDarkMode);
    final userScript = widget.userScript ?? '';
    final localeTag = _localeTag ?? 'en';
...
(function(){
  ...
  Future<void> _applyCefTheme() async {
    final controller = _controller;
    if (controller == null || !_controllerInitialized) return;
    final script = '''
(function(){
  ...
  Future<void> _applyCefLocale() async {
    final controller = _controller;
    if (!mounted || controller == null || !_controllerInitialized) return;
    final strings = _strings;
    final payload = jsonEncode({
      'title': strings.title,
      'connecting': strings.connecting,
    ...
    final webView = controller == null
        ...
        : ValueListenableBuilder<bool>(
            ...
            builder: (context, ready, child) {
              return ready ? controller.webviewWidget : const SizedBox.expand();
            },
          );
    final body = Stack(
      fit: StackFit.expand,
