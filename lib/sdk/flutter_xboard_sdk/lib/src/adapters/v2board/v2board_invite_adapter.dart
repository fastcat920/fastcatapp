...
class V2BoardInviteAdapter implements InviteApi {
  ...
  @override
  Future<List<CommissionDetailModel>> getCommissionDetails(
      {int page = 1, int pageSize = 10}) async {
    final response =
        await _api.fetchCommissionDetails(current: page, pageSize: pageSize);
    if (response.data == null) return [];
  ...
  Future<bool> transferCommissionToBalance(double amount) async {
    final response = await _api.transferCommissionToBalance(
  ...
  InviteCodeModel _mapInviteCode(v2b.InviteCode code) {
    return InviteCodeModel(
      ...
      createdAt:
          DateTime.fromMillisecondsSinceEpoch((code.createdAt ?? 0) * 1000),
      updatedAt:
          DateTime.fromMillisecondsSinceEpoch((code.updatedAt ?? 0) * 1000),
    );
  }
