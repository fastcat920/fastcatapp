...
网关内部同时维护独立的业务后端候选池。客户端只负责切换网关，
每个网关负责提升、熔断和切换自己的业务 API 主备，两层故障转移互不共享状态。

### 1.1 OSS 配置源容灾

- 客户端并发请求全部 OSS 配置源，不再等待前一个源超时后才请求下一个。
- HTTP 请求成功后仍需通过配置结构和完整解析校验；内容损坏时由其他源接管。
- 上次成功源会持久化，并在下次启动时优先发起请求。
- 所有源首轮失败后，才对优先源执行配置的重试次数。
...
每个网关可通过 `DG_BUSINESS_BASE_URLS` 配置逗号分隔的业务后端主备。
成功后端会被提升为当前首选；连续失败达到
`DG_BUSINESS_FAILURE_THRESHOLD`（默认 2）后熔断，经过
`DG_BUSINESS_CIRCUIT_SECONDS`（默认 90 秒）重新获得尝试资格。

GET、HEAD、OPTIONS 和登录请求可在连接错误或 5xx 后切换后端重试。
订单、支付等写请求如果已经收到 HTTP 响应，不会自动跨后端重放，避免重复写入；
失败状态仍会被记录，后续请求会优先使用健康后端。

当备用业务后端正在使用时，网关默认每 30 秒探测一次配置顺序中优先级
更高的后端。主后端连续成功 3 次，并且备用后端已经稳定使用至少 180 秒后，
只把后续新请求切回主后端；在途请求不会取消或重放。以上参数可分别通过
