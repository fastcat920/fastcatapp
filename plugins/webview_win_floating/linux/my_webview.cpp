...
void MyWebView::allowNavigationDecision(int requestId, bool isAllowed) {
    auto it = m_navigationRequestMap.find(requestId);
    if (it == m_navigationRequestMap.end()) return;
    auto decision = it->second;
    m_navigationRequestMap.erase(it);
...
MyWebView::~MyWebView() {
    ...
    for (auto& entry : m_navigationRequestMap) {
        if (entry.second != nullptr) {
            g_object_unref(entry.second);
        }
    }
    m_navigationRequestMap.clear();
